    ...
    *,
    ...
    try:
        ...
        if cancelled_count == 0:
            return APIResponse(
                success=False,
                error="No pending requests found for this conversation"
            )
    ...
    try:
        ...
        if existing_request:
            ...
            update_data = HumanLoopRequestUpdate(
                status="pending" if existing_request.status in ["completed", "cancelled"] else existing_request.status,
                response=None if existing_request.status in ["completed", "cancelled"] else existing_request.response,
                feedback=None if existing_request.status in ["completed", "cancelled"] else existing_request.feedback,
                responded_by=None if existing_request.status in ["completed", "cancelled"] else existing_request.responded_by,
                responded_at=None if existing_request.status in ["completed", "cancelled"] else existing_request.responded_at
            )
